...
// CSS custom-property values are written into a <style> block, where HTML
// entity escaping does not apply — `&#34;` would make the declaration invalid
// and silently drop the font. So these are emitted raw, with the characters
// that could end the declaration or the block stripped instead.
function cssValue(value) {
  let out = String(value == null ? '' : value)
    .replace(/[<>;{}@\\]/g, '')
    // `/*` would comment out the rest of the generated <style> block.
    .replace(/[/*]/g, '')
    .trim();
  // An odd number of quotes leaves a string open and swallows what follows.
  for (const quote of ['"', "'"]) {
    const count = out.split(quote).length - 1;
    if (count % 2 !== 0) out = out.split(quote).join('');
  }
  return out;
}
