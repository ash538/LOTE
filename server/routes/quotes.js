...
router.post('/', (req, res) => {
  const { client_name, client_email, client_company, notes, discount_percent, tax_percent, created_by } = req.body;
  if (!client_name) {
  ...
  db.prepare(`
    INSERT INTO quotes (id, quote_number, client_name, client_email, client_company, notes, discount_percent, tax_percent, created_by)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, quote_number, client_name, client_email || '', client_company || '', notes || '', discount_percent || 0, tax_percent || 0, created_by || '');
...
router.put('/:id', (req, res) => {
  ...
  const { client_name, client_email, client_company, status, notes, discount_percent, tax_percent } = req.body;
  db.prepare(`
    UPDATE quotes SET
    ...
    WHERE id = ?
  `).run(client_name, client_email, client_company, status, notes, discount_percent, tax_percent, req.params.id);
...
router.post('/:id/duplicate', (req, res) => {
  ...
  db.prepare(`
    INSERT INTO quotes (id, quote_number, client_name, client_email, client_company, notes, discount_percent, tax_percent, created_by)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(newId, quote_number, original.client_name, original.client_email, original.client_company, original.notes, original.discount_percent, original.tax_percent, original.created_by);
