...
function ensureAdmin() {
  ...
  // In production an unset ADMIN_PASSWORD gets a random one printed to the
  // logs, so a deploy never ends up with publicly known credentials.
  const generated = !process.env.ADMIN_PASSWORD;
  const password = process.env.ADMIN_PASSWORD
    || (process.env.NODE_ENV === 'production' ? crypto.randomBytes(12).toString('base64url') : 'changeme123');
