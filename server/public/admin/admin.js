...
(function () {
  ...
  function renderShell(main) {
    ...
    var sidebar = el('aside', { class: 'sidebar' }, [
      el('a', { class: 'sidebar-brand', href: '#/', html: 'Site admin<small>' + (state.user ? state.user.email : '') + '</small>' }),
    ]);
  ...
  function viewEnquiries() {
    api('/enquiries').then(function (items) {
      ...
      items.forEach(function (item) {
        ...
        var dl = el('dl', {});
        Object.keys(item.data || {}).forEach(function (label) {
          var value = item.data[label];
          dl.appendChild(el('dt', { text: label }));
          dl.appendChild(el('dd', { text: Array.isArray(value) ? value.join(', ') : value }));
        });
